% Accuracy_Test_26.m is a routine for evaluating accuracy of the solutions    
% to Models 2 and 6: it computes approximation errors in the optimality 
...
%            capital policy functions of N countries and the labor 
%            policy function of country 1;
%            "alpha", "gam", "nu", "b", "phi", "beta", "A", "tau", "rho"
...
function [Errors_mean Errors_max Errors_max_EE Errors_max_MUC Errors_max_MUL Errors_max_RC time_test] = Accuracy_Test_26(k,a,vk,vh,alpha,gam,nu,b,phi,beta,A,tau,rho,vcv,discard)
...
for p = 1:P;                 % For each given point, ...     
       ...
       p                     % Display the point (with the purpose of  
        ...
        h0(1,1) = X0*vh; 
        % Compute a guess on current-period labor of country 1 using the 
        % corresponding labor policy function of country 1; 1-by-1
        ...
        % using iteration on allocation (labor of country 1 is computed by 
        % fixed-point iteration, and labor of the other countries and
        % consumption of all countries are found analytically, given labor 
        % of country 1); 1-by-N
        ...
        h1(1:n_nodes,1) = X1*vh;
        % Compute a guess on next-period labor of country 1 using the
        % corresponding labor policy function of country 1; n_nodes-by-1
        ...
        %---------------------------------------------------------------
        Errors(p,2*N+j) = 1-lambda0./(tau(j)*b(j)*h0(:,j).^(1/nu(j))/A/(1-alpha)./a0(:,j)./k0(:,j).^alpha./h0(:,j).^-alpha); 
        % A unit-free approximation error in the optimality condition w.r.t. 
