% Accuracy_Test_37.m is a routine for evaluating accuracy of the solutions    
% to Models 3 and 7: it computes approximation errors in the optimality 
...
%            "alpha", "gam", "psi", "Le", "phi", "beta", "A", "tau", "rho"
%            and vcv" are the parameters of the model;
...
function [Errors_mean Errors_max Errors_max_EE Errors_max_MUC Errors_max_MUL Errors_max_RC time_test] = Accuracy_Test_37(k,a,vk,vh,alpha,gam,psi,Le,phi,beta,A,tau,rho,vcv,discard)
...
for p = 1:P;                 % For each given point, ...     
       ...
       p                     % Display the point (with the purpose of  
        ...
        % 4.2 Capital and labor choices at t
        % ----------------------------------
        k1 = X0*vk; 
        ...
        h0(1,1:N) = X0*vh; 
        % Compute a guess on current-period labor of N countries using the 
        % second-degree labor policy functions; 1-by-N
        ...
        % using iteration on allocation (fixed-point iteration is performed 
        % on labor of N countries, and consumption of N countries is found 
        % analytically, given labor of N countries); 1-by-N
        
        % 4.3 Capital and labor choices at t+1
        %-------------------------------------
        ...
        h1(1:n_nodes,1:N) = X1*vh;
        % Compute a guess on next-period labor of N countries using the
        % second-degree labor policy functions; n_nodes-by-N
        ...
        % Compute next-period consumption and labor of all countries 
        % using iteration on allocation (labor of N countries is computed  
        % by fixed-point iteration, and consumption of N countries is found
        % analytically, given labor of N countries); n_nodes-by-N
        ...
        %---------------------------------------------------------------
        for j = 1:N
            MUC0j(1,j) = psi*tau(j).*c0(1,j).^(psi*(1-1./gam(j))-1).*(Le-h0(1,j)).^((1-psi)*(1-1./gam(j)));
        ...
        for j = 1:N
            MUC1j(1:n_nodes,j) = psi*tau(j).*c1(1:n_nodes,j).^(psi*(1-1./gam(j))-1).*(Le-h1(1:n_nodes,j)).^((1-psi)*(1-1./gam(j)));
            % Compute a country's marginal utility of consumption multiplied 
            % by its welfare weight
        end
        ...
        %------------------------------------
        for j = 1:N
            Errors(p,j) = 1-weight_nodes'*(beta*lambda1/lambda0/(1+phi*(k1(1,j)/k0(1,j)-1)).*(1+alpha*A*k1(1,j)^(alpha-1)*h1(1:n_nodes,j).^(1-alpha).*a1(1:n_nodes,j)+phi/2*(k2(1:n_nodes,j)/k1(1,j)-1).*(k2(1:n_nodes,j)/k1(1,j)+1)));
        % A unit-free Euler-equation approximation error of country j
        end
        
        % 5.3 Unit-free errors in the optimality conditions w.r.t. consumption
        %---------------------------------------------------------------------
        for j = 1:N;
            Errors(p,N+j) = 1-lambda0./(psi*tau(j).*c0(1,j).^(psi*(1-1./gam(j))-1).*(Le-h0(1,j)).^((1-psi)*(1-1./gam(j))));
        ...
        % of consumption, multiplied by the welfare weight, and the Lagrange 
        % multiplier of the aggregate resource constraint)
        end
        
        % 5.4 Unit-free errors in the optimality conditions w.r.t. labor 
        %---------------------------------------------------------------
        for j = 1:N
            Errors(p,2*N+j) = 1-lambda0*(1-alpha)*A*k0(1,j)^alpha*h0(1,j).^(-alpha).*a0(1,j)./(tau(j)*(1-psi)*c0(1,j).^(psi*(1-1/gam(j))).*(Le-h0(1,j)).^((1-psi)*(1-1/gam(j))-1));
        % A unit-free approximation error in the optimality condition w.r.t. 
        % labor of country j (this condition equates marginal rate of 
        % substitution between labor and consumption, multiplied by the 
        % welfare weight, and the Lagrange multiplier of the aggregate 
        % resource constraint)
        end
        
        % 5.5 Unit-free approximation error in the aggregate resource constraint
        %-----------------------------------------------------------------------
        Errors(p,3*N+1) = 1-(c0(1,1:N) + k1(1,1:N)-k0(1,1:N))*ones(N,1)/((A*k0(1,1:N).^alpha.*h0(1,1:N).^(1-alpha).*a0(1,1:N)-phi/2*(k1(1,1:N)./k0(1,1:N)-1).^2.*k0(1,1:N))*ones(N,1));
        % This error is a unit-free expression of resource constraint (2) 
        % in MMJ (2011)
        
        % 5.6 Approximation errors in the capital-accumulation equation
        %--------------------------------------------------------------
