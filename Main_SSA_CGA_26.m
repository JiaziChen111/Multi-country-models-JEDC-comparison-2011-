% MATLAB software that solves Models 2 and 6 using the stochastic-simulation 
...
% 
% ------------------------------------------------------------------------
% The software that solves Models 2 and 6 uses the following files: 
% ------------------------------------------------------------------------
% 1. "Main_SSA_CGA_26.m"  computes a first-degree polynomial solution under 
%                         SSA and second-degree polynomial solution under  
%                         CGA for Models 2 and 6
% 2. "Iter_On_Aloc_26.m"  performs (fixed-point) iteration-on-allocation on
%                         labor of country 1
% 3. "Simulation_26.m"    simulates a time-series solution given the 
%                         computed policy functions
% 4. "Accuracy_Test_26.m" computes approximation errors in the optimality 
...
% --------------------------------------------------------------
Model = 6;       % Choose the model (either Model=2 or Model=6)
...
% 3. The utility-function parameters, "gamma" and "nu"
% ----------------------------------------------------
gam = 0.25*ones(1,N);                % "gamma" is identical for all countries
nu  = 0.1*ones(1,N);                 % "nu" is identical for all countries
                                     % (Model 2 is the default)
if (Model == 6)&&(N>1);              % In Model 6 with N>1 countries, ...
    gam = (0.25:((1-0.25)/(N-1)):1); % "gamma" is uniformly distributed in  
                                     % the interval [0.25, 1]
    nu  = (0.1:((1-0.1)/(N-1)):1);   % "nu" is uniformly distributed in the  
                                     % interval [0.1, 1]
end
...
for j=1:N
   b(j)   = A^(1-1/gam(j))*(1-alpha); % Utility function parameter "b" 
...
VH = zeros(npol_2d,1,2);    % Matrix of polynomial coefficients of the 
                            % labor policy function of country 1 for two 
                            % solution methods, SSA and CGA; npol_2d-by-1-by-2
...
% To achieve convergence under N>8, one may need to modify the values of 
% the algorithm's parameters (such as hdamp, kdamp, hiter), to refine initial 
% guess and to change the interval in which labor of country 1 is forced 
...
% -------------------------------------------
h_old_1 = ones(T,1);   % A guess on labor series of country 1; this series 
                       % is used both to initialize iteration-on-allocation 
                       % (initially, labor of country 1 is set at the steady-
                       % state, equal to 1) and to check the convergence on  
                       % the subsequent iteration; T-by-1
...
% on-allocation (in Models 2 and 6, labor of country 1 is computed by fixed-
% point iteration, and labor of the other countries and consumption of all 
% countries are found analytically, given labor of country 1)
%--------------------------------------------------------------------------
[c h]  = Iter_On_Aloc_26(h_old_1,k(1:T,:),a(1:T,:),k(2:T+1,:),N,alpha,gam,nu,b,phi,A,tau,hdamp,hiter); 
% T-by-N
...
% ----------------------------------------------------------------------
dif_SSA = (mean(mean(abs(1-k./k_old)))/kdamp+mean(mean(abs(1-h(:,1)./h_old_1)))/hdamp)/2
...
% in condition (42) in MMJ (2011)
%--------------------------------------------------------------------------
for j = 1:N
   y(1:T-1,j) = beta*c(2:T,j).^(-1/gam(j))./c(1:T-1,j).^(-1/gam(j))./(1+phi*(k(2:T,j)./k(1:T-1,j)-1)).*(1+alpha*A*k(2:T,j).^(alpha-1).*h(2:T,j).^(1-alpha).*a(2:T,j)+phi/2*(k(3:T+1,j)./k(2:T,j)-1).*(k(3:T+1,j)./k(2:T,j)+1)).*k(2:T,j);
end  % (T-1)-by-N
...
%-------------------------------------------------------------
h_old_1 = h(:,1);  % Store the series for labor of country 1 to be used 
...
% ---------------------------
vh_1d = x\h(:,1);           % Compute the coefficients of the (first-degree) 
                            % labor policy function of country 1 
VK(1:1+2*N,1:N,1) = vk_1d;  % Store the coefficients of the (first-degree)  
                            % capital policy functions of N countries 
                            % computed by SSA
VH(1:1+2*N,1,1) = vh_1d;    % Store the coefficients of the labor policy 
                            % function of country 1 computed by SSA
...
% current- and next-period labor on the grid
%----------------------------------------------------------------------
for i = 1:n_G;
   ...
   h0_old_1_G(1,1,i) = [1 Grid(i,:)]*vh_1d;
                      % Compute an initial guess on current-period labor
                      % of country 1 on the grid using the SSA solution; 
                      % 1-by-1-by-n_G
   h1_old_1_G(1:n_nodes,1,i) = [ones(n_nodes,1) ones(n_nodes,1)*k1_old_G(:,:,i) a1_G(1:n_nodes,1:N,i)]*vh_1d;
   ...
   % h0_old_1_G and h1_old_1_G are used both for performing iteration-on-
...
while dif_CGA > 1e-7; % The convergence criterion (which is unit free 
    ...
    for i=1:n_G;      % For each grid point (cluster's center) ... 
        ...
        X0 = X0_G(i,:);             % Complete (second-degree) polynomial 
                                    ...
                                    % 1-by-npol_2d 
        h0(1,1) = h0_old_1_G(1,1,i);% A guess on current-period labor of
                                    % country 1 
        h1(1:n_nodes,1) = h1_old_1_G(1:n_nodes,1,i);                          
        ...
        [c0(1,1:N) h0(1,1:N)]  = Iter_On_Aloc_26(h0(1,1),k0,a0,k1,N,alpha,gam,nu,b,phi,A,tau,hdamp,hiter); 
        % Compute current-period consumption and labor of all countries 
        % (labor of country 1 is found by iteration-on-allocation, and labor 
        % of the other countries and consumption of all countries are found 
        % analytically, given labor of country 1); 1-by-N
        ...
        [c1(1:n_nodes,1:N) h1(1:n_nodes,1:N)]  = Iter_On_Aloc_26(h1(1:n_nodes,1),k1_dupl,a1,k2,N,alpha,gam,nu,b,phi,A,tau,hdamp,hiter); 
        % Compute next-period consumption and labor of all countries (labor 
        % of country 1 is found by iteration-on-allocation, and labor of the 
        % other countries and consumption of all countries are found 
        % analytically, given labor of country 1); n_nodes-by-N

        % 18.4. The integral in the right side of the Euler equation (that 
        % is equal to next-period capital) in condition (42) of MMJ (2011)
        %------------------------------------------------------------------        
        for j = 1:N
            k1_hat_G(i,j) = weight_nodes'*(beta*c1(1:n_nodes,j).^(-1/gam(j))/c0(1,j)^(-1/gam(j))/(1+phi*(k1(1,j)/k0(1,j)-1)).*(1+alpha*A*k1(1,j)^(alpha-1).*h1(1:n_nodes,j).^(1-alpha).*a1(1:n_nodes,j)+phi/2*(k2(1:n_nodes,j)/k1(1,j)-1).*(k2(1:n_nodes,j)/k1(1,j)+1))*k1(1,j));
        ...
        %----------------------------------
        h0_new_1_G(1,1,i) = h0(1,1);
        % Store current-period labor of country 1 to be used as an input 
        % for performing iteration-on-allocation and for checking the
        % convergence on the subsequent iteration; 1-by-1-by-n_G 

        h1_new_1_G(1:n_nodes,1,i) = h1(1:n_nodes,1);   
        % Store next-period labor of country 1 to be used as an input for 
...
% -------------------------------------------------------------------------
dif_CGA = (mean(mean(abs(1-k1_new_G./k1_old_G)))/kdamp+mean(mean(abs(1-h0_new_1_G./h0_old_1_G)))/hdamp)/2
                   % The convergence criterion is adjusted to the damping 
                   % parameters as described in MMJ (2011)    
...
%-------------------------------------------------------------------------
h0_old_1_G = h0_new_1_G; 
                   % Store current-period labor of country 1 on the grid to 
                   % be used as an input for performing iteration- 
                   % on-allocation and for checking the convergence on 
                   % the subsequent iteration
h1_old_1_G = h1_new_1_G; 
                   % Store next-period labor of country 1 on the grid
...
VH(:,1,2) = vh_2d;   % Store the coefficients of the labor policy 
...
for m = 1:2 
    ...
    % all zeros)
    %--------------------------------------------------------------------
    [c_sim h_sim k_sim] = Simulation_26(VK(:,:,m),VH(:,:,m),a_sim,k0_sim,alpha,gam,nu,b,phi,A,tau);
    
    % Errors across 1,000 points on radius r=0.01
    % -------------------------------------------
    discard = 0;    % Do not discard observations
    [Errors_mean(m,1),Errors_max(m,1),Errors_max_EE(m,1),Errors_max_MUC(m,1),Errors_max_MUL(m,1),Errors_max_RC(m,1),time_test(m,1)] = Accuracy_Test_26(k_r001,a_r001,VK(:,:,m),VH(:,1,m),alpha,gam,nu,b,phi,beta,A,tau,rho,vcv,discard);

    % Errors across 1,000 points on radius r=0.1
    % ------------------------------------------
    discard = 0; 
    [Errors_mean(m,2),Errors_max(m,2),Errors_max_EE(m,2),Errors_max_MUC(m,2),Errors_max_MUL(m,2),Errors_max_RC(m,2),time_test(m,2)] = Accuracy_Test_26(k_r01,a_r01,VK(:,:,m),VH(:,1,m),alpha,gam,nu,b,phi,beta,A,tau,rho,vcv,discard);

    % Errors across 1,000 points on radius r=0.3
    % ------------------------------------------
    discard = 0; 
    [Errors_mean(m,3),Errors_max(m,3),Errors_max_EE(m,3),Errors_max_MUC(m,3),Errors_max_MUL(m,3),Errors_max_RC(m,3),time_test(m,3)] = Accuracy_Test_26(k_r03,a_r03,VK(:,:,m),VH(:,1,m),alpha,gam,nu,b,phi,beta,A,tau,rho,vcv,discard);
    ...
    [Errors_mean(m,4),Errors_max(m,4),Errors_max_EE(m,4),Errors_max_MUC(m,4),Errors_max_MUL(m,4),Errors_max_RC(m,4),time_test(m,4)] = Accuracy_Test_26(k_sim(1:T_sim,:),a_sim(1:T_sim,:),VK(:,:,m),VH(:,1,m),alpha,gam,nu,b,phi,beta,A,tau,rho,vcv,discard);
