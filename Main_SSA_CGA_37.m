% MATLAB software that solves Models 3 and 7 using the stochastic-simulation 
...
% 
% ------------------------------------------------------------------------
% The software that solves Models 3 and 7 uses the following files: 
% ------------------------------------------------------------------------
% 1. "Main_SSA_CGA_37.m"  computes a first-degree polynomial solution under 
%                         SSA and second-degree polynomial solution under  
%                         CGA for Models 3 and 7
% 2. "Iter_On_Aloc_37.m"  performs (fixed-point) iteration-on-allocation on
%                         labor of N countries
% 3. "Simulation_37.m"    simulates a time-series solution given the 
%                         computed policy functions
% 4. "Accuracy_Test_37.m" computes approximation errors in the optimality 
...
% --------------------------------------------------------------
Model = 7;       % Choose the model (either Model=3 or Model=7)
...
gam  = 0.25*ones(1,N);               % "gamma" is identical for all countries
                                     % (Model 3 is the default)
if (Model == 7)&&(N>1);              % In Model 7 with N>1 countries, ...
   gam  = (0.25:((1-0.25)/(N-1)):1); % "gamma" is uniformly distributed across 
                                     % countries in the interval [0.25, 1]
end

% 4. The normalizing constant, A, utility parameter, psi, and welfare 
% weights, tau 
% ------------------------------------------------------------------
A       = (1-beta)/alpha/beta;        % The constant in the production function 
psi = 1/(1-alpha)/(1/(1-alpha)+Le-1); % The utility-function parameter "psi" 
for j = 1:N
   tau(j) = A^(psi/gam(j))*(Le-1)^((1-psi)/gam(j));  
...
% on-allocation (in Models 3 and 7, labor of N countries is computed by  
% fixed-point iteration, and consumption of N countries is found analytically, 
% given labor of N countries)
%----------------------------------------------------------------------------
[c h]  = Iter_On_Aloc_37(h_old,k(1:T,:),a(1:T,:),k(2:T+1,:),N,alpha,gam,psi,phi,A,Le,tau,hdamp,hiter); 
% T-by-N
...
% in condition (43) in MMJ (2011)
%--------------------------------------------------------------------------
for j = 1:N
   y(1:T-1,j) = beta*c(2:T,j).^(psi*(1-1/gam(j))-1).*(Le-h(2:T,j)).^((1-psi)*(1-1/gam(j)))./c(1:T-1,j).^(psi*(1-1/gam(j))-1)./(Le-h(1:T-1,j)).^((1-psi)*(1-1/gam(j)))./(1+phi*(k(2:T,j)./k(1:T-1,j)-1)).*(1+alpha*A*k(2:T,j).^(alpha-1).*h(2:T,j).^(1-alpha).*a(2:T,j)+phi/2*(k(3:T+1,j)./k(2:T,j)-1).*(k(3:T+1,j)./k(2:T,j)+1)).*k(2:T,j);
end  % (T-1)-by-N
...
while dif_CGA > 1e-7; % The convergence criterion (which is unit free 
    ...
    for i=1:n_G;      % For each grid point (cluster's center) ... 
        ...
        [c0(1,1:N) h0(1,1:N)]  = Iter_On_Aloc_37(h0(1,1:N),k0,a0,k1,N,alpha,gam,psi,phi,A,Le,tau,hdamp,hiter); 
        % Compute current-period consumption and labor of all countries 
        % (labor of N countries is computed by fixed-point iteration, and 
        ...
        [c1(1:n_nodes,1:N) h1(1:n_nodes,1:N)]  = Iter_On_Aloc_37(h1(1:n_nodes,1:N),k1_dupl,a1,k2,N,alpha,gam,psi,phi,A,Le,tau,hdamp,hiter); 
        % Compute next-period consumption and labor of all countries (labor 
        % of N countries is computed by fixed-point iteration, and consumption 
        % of N countries is found analytically, given labor of N countries); 
        % n_nodes-by-N

        % 18.4. The integral in the right side of the Euler equation (that 
        % is equal to next-period capital) in condition (43) of MMJ (2011)
        %------------------------------------------------------------------        
        for j = 1:N
            k1_hat_G(i,j) = weight_nodes'*(beta*c1(1:n_nodes,j).^(psi*(1-1/gam(j))-1).*(Le-h1(1:n_nodes,j)).^((1-psi)*(1-1/gam(j)))./c0(1,j).^(psi*(1-1/gam(j))-1)./(Le-h0(1,j)).^((1-psi)*(1-1/gam(j)))/(1+phi*(k1(1,j)/k0(1,j)-1)).*(1+alpha*A*k1(1,j)^(alpha-1).*h1(1:n_nodes,j).^(1-alpha).*a1(1:n_nodes,j)+phi/2*(k2(1:n_nodes,j)/k1(1,j)-1).*(k2(1:n_nodes,j)/k1(1,j)+1))*k1(1,j));
...
for m = 1:2 
    ...
    % all zeros)
    %--------------------------------------------------------------------
    [c_sim h_sim k_sim] = Simulation_37(VK(:,:,m),VH(:,:,m),a_sim,k0_sim,alpha,gam,psi,Le,phi,A,tau);
    
    % Errors across 1,000 points on radius r=0.01
    % -------------------------------------------
    discard = 0;    % Do not discard observations
    [Errors_mean(m,1),Errors_max(m,1),Errors_max_EE(m,1),Errors_max_MUC(m,1),Errors_max_MUL(m,1),Errors_max_RC(m,1),time_test(m,1)] = Accuracy_Test_37(k_r001,a_r001,VK(:,:,m),VH(:,:,m),alpha,gam,psi,Le,phi,beta,A,tau,rho,vcv,discard);

    % Errors across 1,000 points on radius r=0.1
    % ------------------------------------------
    discard = 0; 
    [Errors_mean(m,2),Errors_max(m,2),Errors_max_EE(m,2),Errors_max_MUC(m,2),Errors_max_MUL(m,2),Errors_max_RC(m,2),time_test(m,2)] = Accuracy_Test_37(k_r01,a_r01,VK(:,:,m),VH(:,:,m),alpha,gam,psi,Le,phi,beta,A,tau,rho,vcv,discard);

    % Errors across 1,000 points on radius r=0.3
    % ------------------------------------------
    discard = 0; 
    [Errors_mean(m,3),Errors_max(m,3),Errors_max_EE(m,3),Errors_max_MUC(m,3),Errors_max_MUL(m,3),Errors_max_RC(m,3),time_test(m,3)] = Accuracy_Test_37(k_r03,a_r03,VK(:,:,m),VH(:,:,m),alpha,gam,psi,Le,phi,beta,A,tau,rho,vcv,discard);
    ...
    [Errors_mean(m,4),Errors_max(m,4),Errors_max_EE(m,4),Errors_max_MUC(m,4),Errors_max_MUL(m,4),Errors_max_RC(m,4),time_test(m,4)] = Accuracy_Test_37(k_sim,a_sim,VK(:,:,m),VH(:,:,m),alpha,gam,psi,Le,phi,beta,A,tau,rho,vcv,discard);
