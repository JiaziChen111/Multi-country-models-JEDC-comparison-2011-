% Accuracy_Test_48.m is a routine for evaluating accuracy of the solutions    
% to Models 4 and 8: it computes approximation errors in the optimality 
...
%            "alpha", "gam", "xi", "mu", "b", "Le", "phi", "beta", "A",
%            "tau", "rho" and vcv" are the parameters of the model;
...
function [Errors_mean Errors_max Errors_max_EE Errors_max_MUC Errors_max_MUL Errors_max_RC time_test] = Accuracy_Test_48(k,a,vk,vh,alpha,gam,xi,mu,b,Le,phi,beta,A,tau,rho,vcv,discard)
...
for p = 1:P;                 % For each given point, ...     
       ...
       p                     % Display the point (with the purpose of  
        ...
        %---------------------------------------------------------------
        for j = 1:N
            MUC0j(1,j) = tau(j)*(c0(1,j).^(1-1/xi(j))+b(j)*(Le-h0(1,j)).^(1-1/xi(j))).^((1/xi(j)-1/gam(j))/(1-1/xi(j))).*c0(1,j).^(-1/xi(j));
        ...
        % multiplied by its welfare weight; to compute the Lagrange multiplier, 
        % we average across N countries; 1-by-1

        for j = 1:N
            MUC1j(1:n_nodes,j) = tau(j)*(c1(1:n_nodes,j).^(1-1/xi(j))+b(j)*(Le-h1(1:n_nodes,j)).^(1-1/xi(j))).^((1/xi(j)-1/gam(j))/(1-1/xi(j))).*c1(1:n_nodes,j).^(-1/xi(j));
            % Compute a country's marginal utility of consumption multiplied 
            % by its welfare weight
        end
        ...
        % of consumption multiplied by its welfare weight; to compute the 
        % Lagrange multiplier, we average across N countries; 1-by-n_nodes
                
        % 5.2 Unit-free Euler-equation errors
        %------------------------------------
        for j = 1:N
            Errors(p,j) = 1-weight_nodes'*(beta*lambda1/lambda0/(1+phi*(k1(1,j)/k0(1,j)-1)).*(1+alpha*A*a1(1:n_nodes,j).*k1(1,j).^(mu(j)-1).*(alpha*k1(1,j).^mu(j)+(1-alpha)*h1(1:n_nodes,j).^mu(j)).^(1/mu(j)-1)+phi/2*(k2(1:n_nodes,j)/k1(1,j)-1).*(k2(1:n_nodes,j)/k1(1,j)+1)));
        ...
        %---------------------------------------------------------------------
        for j = 1:N;
            Errors(p,N+j) = 1-lambda0./(tau(j)*(c0(1,j).^(1-1/xi(j))+b(j)*(Le-h0(1,j)).^(1-1/xi(j))).^((1/xi(j)-1/gam(j))/(1-1/xi(j))).*c0(1,j).^(-1/xi(j)));
        ...
        %---------------------------------------------------------------
        for j = 1:N
            Errors(p,2*N+j) = 1-lambda0*(1-alpha)*A*a0(1,j).*h0(1,j).^(mu(j)-1).*(alpha*k0(1,j).^mu(j)+(1-alpha)*h0(1,j).^mu(j)).^(1/mu(j)-1)./(tau(j)*(c0(1,j).^(1-1/xi(j))+b(j)*(Le-h0(1,j)).^(1-1/xi(j))).^((1/xi(j)-1/gam(j))/(1-1/xi(j))).*b(j)*(Le-h0(1,j)).^(-1/xi(j)));
        % A unit-free approximation error in the optimality condition w.r.t. 
        ...
        %-----------------------------------------------------------------------
        Errors(p,3*N+1) = 1-(c0(1,1:N)+ k1(1,1:N)-k0(1,1:N))*ones(N,1)/((A*a0(1,1:N).*(alpha*k0(1,1:N).^mu(1:N)+(1-alpha)*h0(1,1:N).^mu(1:N)).^(1./mu(1:N))-phi/2*(k1(1,:)./k0(1,:)-1).^2.*k0(1,:))*ones(N,1));
